...
def run_model(X_txt, y_val):

    # Charger le modèle
    model = joblib.load('../../models/rf_combi.pkl')
    
    # Utiliser le modèle chargé pour faire des prédictions
    y_pred = model.predict(X_txt)
...
if __name__ == "__main__":
    ...
    with h5py.File('val_ml_data.h5', 'r') as hf:
        X_val_combined = hf['X_val_combined'][:]
        y_val = hf['y_val'][:]

    report, cm = run_model(X_val_combined, y_val)
